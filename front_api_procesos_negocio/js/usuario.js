...
function listarUsuarios() {
  ...
  fetch(urlApi + "/listusers", settings)
    ...
    .catch(function (error) {
      ...
      alertas(
        "Agrege un usuario dandole click al bot&oacute;n ya que est&aacute; base de datos est&aacute; vac&iacute;a!",
        2
      );
    });
}
...
function eliminaUsuario(id) {
  ...
  fetch(urlApi + "/user/" + id, settings).then((data) => {
    console.log(data); // JSON data parsed by `data.json()` call
    listarUsuarios();
    alertas(
      "Se ha eliminado el usuario exitosamente, Si no se elimino el usuario es por que esta relacionado a uno o muchos articulos primero elimine esos articulos para eliminar el usuario!",
      2
    );
  });
}
      ...
      if (usuario) {
        ...
        cadena = `
                ...
                <form action="" method="post" id="myForm">
                    ...
                    <input type="text" class="form-control" name="document" id="document" required value="${
                      usuario.document
                    }"> <br>
                    ...
                    <input type="date" class="form-control" name="date" id="date" required value="${usuario.date.substr(
                      0,
                      10
                    )}"> <br>
                    <label for="phone" class="form-label">Direction</label>
                    <input type="text" class="form-control" name="direction" id="direction" required value="${
                      usuario.direction
                    }"> <br>
      ...
      if (usuario) {
        cadena = `
                ...
                <ul class="list-group">
                    ...
                    <li class="list-group-item">Fecha de nacimiento: ${usuario.date.substr(
                      0,
                      10
                    )}</li>
                    <li class="list-group-item">Direcci&oacute;n: ${
                      usuario.direction
                    }</li>
